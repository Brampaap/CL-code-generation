...
def extract_tokenized(
    dataset: list[dict], lang_type: str = "ru"
) -> tuple[list[list[str]], list[list[str]]]:
    tokenized_inputs = list()
    tokenized_target = list()

    for item in dataset:
        tokenized_inputs.append(item["question_toks"][lang_type])
        tokenized_target.append(item["query_toks"][lang_type])

    return tokenized_inputs, tokenized_target


class Text2SQLDataModule(pl.LightningDataModule):
    def __init__(self, batch_size: int):
        super(Text2SQLDataModule, self).__init__()
    ...
    def __get_train_dev_test__(
        ...
        proportions = [
            1.0 - PAUQ_CONFIG["train"]["dev_size"],
            PAUQ_CONFIG["train"]["dev_size"],
        ]
        lengths = [int(p * len(train)) for p in proportions]
        train_splitted, dev = random_split(dataset=train, lengths=lengths)
    ...
    def setup(self, stage=None):
        train_full, test = dataset_utils.load(PAUQ_CONFIG)
